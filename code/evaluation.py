...
def compare(predict, gold, config, show_rate=None, simple=True):
    ...
    f1, precision, recall, _, _, _ = compare_(predict, gold, 'ALL', config, show_rate, strict=True)
    if simple:
...
def compare_(predict, gold, name, config, show_rate=None, is_show=True, strict=True):
    predict_number = 0
    gold_number = 0
    correct_num = 0
    for p, g in zip(predict, gold):
